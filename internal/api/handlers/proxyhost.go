...
func (h *H) CreateProxyHost(w http.ResponseWriter, r *http.Request) {
	...
	pemPath := req.SSLPEMPath
	if pemPath == "" && req.SSLEnabled {
...
func (h *H) UpdateProxyHost(w http.ResponseWriter, r *http.Request) {
	...
	updatePEMPath := req.SSLPEMPath
	if updatePEMPath == "" && req.SSLEnabled {
		updatePEMPath = h.defaultPEMPath
	}
...
func (h *H) TestProxyHost(w http.ResponseWriter, r *http.Request) {
	...
	// referenceHost is the domain the operator is currently browsing OffDock
	// through (from the Host header). Other proxy-host domains should point to
	// the same target as this known-working domain.
	referenceHost := r.Host
	if idx := strings.LastIndex(referenceHost, ":"); idx != -1 {
		referenceHost = referenceHost[:idx]
	}

	// ServerIP is the machine's actual primary IPv4 — what an A record should
	// point to for a directly-reachable (non-proxied) setup.
	res := result{ServerIP: primaryServerIP()}
	if res.ServerIP == "" {
		res.ServerIP = referenceHost
	}

	var hints []string

	// Resolve the reference domain to its IP set so we can tell whether the
	// target resolves to the same place (handles Cloudflare / proxied DNS where
	// the public A record points at a CDN, not the origin IP).
	dnsCtx0, dnsCancel0 := context.WithTimeout(r.Context(), 5*time.Second)
	refAddrs, _ := net.DefaultResolver.LookupHost(dnsCtx0, referenceHost)
	dnsCancel0()
	expectedSet := make(map[string]bool, len(refAddrs)+1)
	for _, a := range refAddrs {
		expectedSet[a] = true
	}
	if ip := primaryServerIP(); ip != "" {
		expectedSet[ip] = true
	}
	if net.ParseIP(referenceHost) != nil {
		expectedSet[referenceHost] = true
	}
	// Human-readable description of where the target *should* resolve.
	expectedDesc := res.ServerIP
	if len(refAddrs) > 0 && net.ParseIP(referenceHost) == nil {
		expectedDesc = fmt.Sprintf("the same as %s (%s)", referenceHost, strings.Join(refAddrs, ", "))
	}
	...
	if dnsErr != nil {
		...
		hints = append(hints, fmt.Sprintf("DNS: %q has no A record yet — add one pointing to %s (the same target as %s).", host.Domain, res.ServerIP, referenceHost))
	} else {
		...
		if !res.DNSPointsHere {
			hints = append(hints, fmt.Sprintf("DNS: %q resolves to %s, but it should resolve to %s — update the A record.", host.Domain, strings.Join(addrs, ", "), expectedDesc))
		}
	}
...
// primaryServerIP returns the machine's primary non-loopback IPv4 address,
// or "" if none can be determined.
func primaryServerIP() string {
