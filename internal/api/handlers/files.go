...
func (h *H) FileWrite(w http.ResponseWriter, r *http.Request) {
	...
	if isSensitivePath(path) || isBlockedPath(path) {
		writeError(w, http.StatusForbidden, "cannot write to this path")
		return
	}
...
// FileImport copies or moves an uploaded file from /var/offdock/uploads/ to any
// writable destination path, or copies any source path to a destination.
// action: "copy" (default) or "move".
func (h *H) FileImport(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Source string `json:"source"` // absolute path or filename in /var/offdock/uploads/
		Dest   string `json:"dest"`   // absolute destination path (file or directory)
		Action string `json:"action"` // "copy" or "move"
	}
	if err := decodeJSON(r, &req); err != nil || req.Source == "" || req.Dest == "" {
	...
	// Resolve source: if it's not an absolute path, look in uploads dir.
	src := req.Source
	if !filepath.IsAbs(src) {
		src = filepath.Join("/var/offdock/uploads", filepath.Base(src))
	}
	src = cleanPath(src)

	info, err := os.Stat(src)
	if err != nil {
		writeError(w, http.StatusNotFound, "source file not found: "+src)
		return
	}
	if info.IsDir() {
		writeError(w, http.StatusBadRequest, "source must be a file, not a directory")
		return
	}

	// Resolve dest: if it points to a directory, use source filename inside it.
	...
	if err := os.MkdirAll(filepath.Dir(dest), 0o755); err != nil {
		writeError(w, http.StatusInternalServerError, "create destination dir: "+err.Error())
		return
	}
	...
	if req.Action == "move" {
		if err := os.Rename(src, dest); err != nil {
			...
			if err2 := copyFile(src, dest); err2 != nil {
				writeError(w, http.StatusInternalServerError, "move file: "+err2.Error())
				return
			}
		...
		if err := copyFile(src, dest); err != nil {
			writeError(w, http.StatusInternalServerError, "copy file: "+err.Error())
			return
		}
	}
...
// ListUploads returns the files currently in /var/offdock/uploads/.
func (h *H) ListUploads(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		writeError(w, http.StatusInternalServerError, "list uploads: "+err.Error())
		return
	}
