    ...
    <div className="shrink-0 border-b border-slate-800 bg-slate-950/60">
      <button
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-300">Docker Networking Guide — How OffDock Routing Works</span>
        </div>
        <span className="text-slate-600 text-xs">{open ? '▲ hide' : '▼ show'}</span>
      </button>

      {open && (
        <div className="px-5 pb-5 space-y-5 text-xs text-slate-400 leading-relaxed">

          {/* Flow diagram */}
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 font-mono text-xs">
            <p className="text-slate-500 mb-3">Request flow for multiple apps on one server:</p>
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <span className="text-blue-400">Browser</span>
                <span className="text-slate-700">──── Host: grafana.ao.az ───→</span>
              ...
              <div className="flex items-center gap-2">
                <span className="text-blue-400">Browser</span>
                <span className="text-slate-700">──── Host: modtube.local ────→</span>
                <span className="text-yellow-400">nginx:80</span>
                <span className="text-slate-700">──→</span>
                <span className="text-green-400">offdock-external</span>
                <span className="text-slate-700">──→</span>
              ...
              <div className="flex items-center gap-2">
                <span className="text-blue-400">Browser</span>
                <span className="text-slate-700">──── Host: (raw IP) ─────────→</span>
                <span className="text-yellow-400">nginx:80</span>
                <span className="text-slate-700">──→</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-blue-950/20 border border-blue-900/40 rounded-xl p-4 space-y-2">
              ...
              <p>Connect containers that nginx needs to reach — your <strong className="text-slate-300">web apps, APIs, dashboards</strong>.</p>
              <p className="text-slate-500">nginx uses Docker's embedded DNS to resolve container names to IPs. The container name in your upstream_host field is resolved inside this network.</p>
              <div className="mt-2 bg-slate-900/60 rounded-lg p-2.5 font-mono text-[10px] space-y-1">
                <p className="text-slate-500">Example: upstream is "grafana"</p>
            ...
            <div className="bg-slate-800/20 border border-slate-700/40 rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-300">offdock-internal</span>
              </div>
              <p>Connect <strong className="text-slate-300">backend services</strong> (databases, Redis, message queues) that should NOT be directly reachable from nginx or the internet.</p>
              <p className="text-slate-500">Your app containers can be on BOTH networks: external (so nginx can reach them) and internal (so they can reach the database).</p>
              <div className="mt-2 bg-slate-900/60 rounded-lg p-2.5 font-mono text-[10px] space-y-1">
                <p className="text-slate-500">Example: modtube app container</p>
          ...
          <div className="space-y-2">
            <p className="font-semibold text-slate-300">Common setup recipes:</p>
            <div className="space-y-1.5 font-mono text-[10px]">
              <div className="bg-slate-900 rounded-lg px-3 py-2 border border-slate-800">
                <p className="text-slate-500 mb-1"># Simple web app — connect only to external</p>
                <p className="text-blue-400">docker network connect offdock-external grafana</p>
              </div>
              <div className="bg-slate-900 rounded-lg px-3 py-2 border border-slate-800">
                <p className="text-slate-500 mb-1"># App + database — app on both, db on internal only</p>
              ...
              <div className="bg-slate-900 rounded-lg px-3 py-2 border border-slate-800">
                <p className="text-slate-500 mb-1"># Test routing by Host header (from any machine on the network)</p>
...
function NetworkCard({
  ...
  if (isOffdock && net.Name === 'offdock-internal') { borderColor = 'border-slate-700'; dotColor = 'bg-slate-400'; titleColor = 'text-slate-300' }

  return (
    <div className={`border ${borderColor} rounded-xl bg-slate-900/40 overflow-hidden`}>
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between gap-3 border-b border-slate-800/60">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded border border-slate-700 text-slate-500 bg-slate-800/60 shrink-0">{net.Driver}</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {!isSystem && (
            <button
              ...
              className="text-slate-700 hover:text-red-400 transition-colors text-xs disabled:opacity-30 disabled:cursor-not-allowed">✕</button>
          )}
        </div>
      </div>
      ...
      <div className="divide-y divide-slate-800/40">
        {containers.length === 0 ? (
          <div className="px-4 py-3 text-xs text-slate-700 italic">No containers connected</div>
        ) : (
          containers.map(c => (
            <div key={c.id} className="flex items-center gap-2.5 px-4 py-2.5 group">
              ...
              <button
                ...
                title="Disconnect"
                className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 transition-all disabled:opacity-30 text-xs">
                ✕
              </button>
            </div>
          ))
      ...
      {/* Connect form */}
      {!isSystem && (
        <div className="px-4 py-3 border-t border-slate-800/60 flex items-center gap-2">
          <select
            value={selected}
            onChange={e => setSelected(e.target.value)}
            className="flex-1 appearance-none bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500/40">
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 flex items-center gap-3 px-5 py-3 border-b border-slate-800 bg-slate-950">
        <h1 className="text-sm font-semibold text-slate-100 flex-1">Docker Networks</h1>
        {msg && (
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        {loading ? (
          <div className="text-center py-12 text-slate-600 text-sm">Loading networks…</div>
        ) : (
          <>
            {/* OffDock managed */}
            <div className="space-y-3">
              <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                ...
                OffDock Networks
                <span className="text-slate-700 font-normal normal-case">(created automatically)</span>
              </h2>
              {offdockNets.length === 0 ? (
                <p className="text-xs text-slate-700 italic">Not created yet — start nginx to auto-create them.</p>
              ) : (
            ...
            {systemNets.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                  ...
                  System Networks
                  <span className="text-slate-700 font-normal normal-case">(read-only)</span>
                </h2>
