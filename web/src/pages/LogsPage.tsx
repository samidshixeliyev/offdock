...
export default function LogsPage() {
  ...
  useEffect(() => {
    ...
    api.listContainers(id).then(c => {
      setContainers(c)
      if (!selected && c.length > 0) setSelected(c[0].Names)
    })
  }, [id])
  ...
  useEffect(() => {
    ...
    const es = new EventSource(
      `/api/v1/projects/${id}/containers/${encodeURIComponent(selected)}/logs?tail=200`
    )
    esRef.current = es
    es.onmessage = e => {
      try {
  ...
  return (
    <div className="p-6 flex flex-col h-[calc(100vh-2rem)]">
      ...
      <div
        ref={logRef}
        className="flex-1 font-mono text-xs bg-gray-950 border border-gray-800 rounded-xl p-4 overflow-y-auto"
      >
        {filtered.length === 0 && (
