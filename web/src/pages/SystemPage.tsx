  ...
  return (
    <div className="card">
      ...
      <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
        <div
          className={clsx('h-full rounded-full transition-all duration-500', pct > 85 ? 'bg-red-500' : pct > 60 ? 'bg-yellow-500' : 'bg-blue-500')}
    ...
    <div className="p-6 max-w-5xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <Gauge
          label="RAM"
          pct={ramPct}
          sub={`${humanGB(stats.ram_used_bytes)} used / ${humanGB(stats.ram_total_bytes)}`}
        />
        <Gauge
          label="Disk"
          pct={diskPct}
          sub={`${humanGB(stats.disk_used_bytes)} used / ${humanGB(stats.disk_total_bytes)}`}
        />
      </div>
