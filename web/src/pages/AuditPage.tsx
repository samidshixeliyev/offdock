    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-7xl">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div>
          ...
          <select
            ...
            className="bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500"
          >
      ...
      {loading ? (
        <div className="card p-0 overflow-hidden">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="border-b border-gray-800/50 last:border-0 px-4 py-3 flex gap-4 animate-pulse">
              <div className="h-3 bg-gray-800 rounded w-32 shrink-0" />
              <div className="h-3 bg-gray-800 rounded w-20 shrink-0" />
              <div className="h-3 bg-gray-800 rounded w-16 shrink-0" />
              <div className="h-3 bg-gray-800 rounded flex-1" />
              <div className="h-3 bg-gray-800 rounded w-24 shrink-0" />
            </div>
          ))}
        </div>
      ) : events.length === 0 ? (
        <div className="card text-center py-16 text-gray-600 text-sm border-dashed">
          ...
          <p className="text-gray-500 text-sm mb-1">No audit events found</p>
          <p className="text-gray-700 text-xs">Try adjusting the filters above</p>
        </div>
      ) : (
        <div className="card p-0 overflow-x-auto">
          <table className="w-full text-sm min-w-[900px]">
            ...
            <tbody>
              {events.map(ev => (
                <tr key={ev.id} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                  <td className="px-4 py-2.5 text-xs text-gray-500 font-mono whitespace-nowrap">
                    {formatTime(ev.created_at)}
                  </td>
                  <td className="px-4 py-2.5 text-xs text-gray-300 font-medium whitespace-nowrap">
                    {ev.username || <span className="text-gray-600 font-normal">—</span>}
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-xs whitespace-nowrap">
                    <span className="text-gray-400">{ev.resource_type || '—'}</span>
                    {ev.resource_name && (
                      <span className="text-gray-300 ml-1.5 font-mono">/ {ev.resource_name}</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-xs text-gray-400 max-w-md">
                    <span className="truncate block" title={ev.details}>
                      {ev.details || <span className="text-gray-700">—</span>}
                    </span>
                  </td>
