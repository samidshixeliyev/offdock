...
// ─── SVG icon set (Heroicons-style, 16px) ─────────────────────────────────────
type IconName =
  | 'dashboard' | 'images' | 'usb' | 'proxy' | 'networks' | 'volumes'
  | 'files' | 'system' | 'containers' | 'terminal' | 'users' | 'audit'
  | 'logout' | 'chevron'
...
const navMain: NavItem[] = [
  ...
  { to: '/usb',        label: 'USB Import',    icon: 'usb' },
...
// Build a friendly breadcrumb-style label from the current path.
function breadcrumbFor(pathname: string): string {
  if (pathname === '/' || pathname === '') return 'Dashboard'
  const segs = pathname.split('/').filter(Boolean)
  // Projects: /projects/new, /projects/:id, /projects/:id/<tab>
  if (segs[0] === 'projects') {
...
function SidebarLink({ item, delay }: { item: NavItem; delay: number }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 opacity-0 animate-slideInLeft',
          isActive
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
          <div>
            ...
            <div className="space-y-1">
              {navMain.map((item, i) => (
                <SidebarLink key={item.to} item={item} delay={30 + i * 30} />
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-1">
              {navSystem.map((item, i) => (
                <SidebarLink key={item.to} item={item} delay={30 + (navMain.length + i) * 30} />
              ))}
            </div>
          </div>
        </nav>
      ...
      <main className="flex-1 overflow-hidden flex flex-col min-h-0 bg-slate-950">
        ...
        <header className="h-14 border-b border-slate-800/50 bg-slate-950/80 backdrop-blur flex items-center px-6 shrink-0">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Icon name="chevron" className="w-3 h-3 text-slate-700" />
